...
public static class ImageChunkCodec
{
    ...
    /// <summary><paramref name="chunkSize"/> is the tile's side length — the decompressed length is
    /// always <c>chunkSize * chunkSize</c>, since every chunk is stored at its image's fixed tile size
    /// regardless of how much of it falls inside the canvas.</summary>
    public static byte[] Decode(byte format, byte[] bytes, int chunkSize)
    {
        ...
        var pixels = new byte[chunkSize * chunkSize];
