...
public static class EnvironmentBlender
{
    ...
    /// <see cref="IEnvironmentSource.WeightAt"/> and folded in weakest to strongest, so the strongest
    /// has the final say where two overlap — Noggit's own ordering, expressed directly instead of via
    /// a distance sort.
    /// </summary>
    public static Result Blend(IEnumerable<IEnvironmentSource> sources, Vector3 focus, EnvironmentTime time)
    {
        ...
        weighted.Sort((a, b) => a.Weight.CompareTo(b.Weight));
